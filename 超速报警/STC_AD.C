...
#define VCC 4.670   // 存放用万用表实测的单片机供电电压
...
// 调用时间，可把这2行剪切到主程序中去。
	P1ASF = 0x00;
	P1ASF|=0x08;			// 选择P1.3作为AD转换通道，0x08= 0000 1000
	P1ASF&=~0xfb;			// 选择P1.3作为AD转换通道，0x08= 0000 1000	
	ADC_CONTR=0xE3;         // 选择P1.3作为AD转换通道，最高转换速度，清转换完成标志。
...
void ADC(void) interrupt 5
{
	...
	ADC_CONTR&=0xE7;     	  		// 将ADC_FLAG清0， 0xE7=1110 0111，ADC_FLAG=0，ADC_START=0。	
	//ADC_CONTR&=~ADC_FLAG;
	//ADC_CONTR&=~ADC_START;
...
void ADCDisplay()
{
	...
	if(ADvalue<100)
	{
		Display_String(3,"空气质量：优");
	}
	else if(ADvalue<300)
	{
		Display_String(3,"空气质量：良");
	}
	else if(ADvalue<500)
	{
		Display_String(3,"空气质量：中");
	}
	else
	{
		Display_String(3,"空气质量：差");
	}
